...
动态靶机和动态附件要求应用能够访问 Kubernetes API，并拥有创建、查询、删除相关资源的权限。Helm Chart 会创建默认
ClusterRole；如果自行部署，需要按 Chart 中的权限范围配置 RBAC。

KubeVirt VM 靶机没有独立开关。题目 compose 中的 service 设置 `x-kubevirt: true` 后，平台会在 VPC 模式下创建 KubeVirt
`VirtualMachine`。因此自行部署时还需要安装 KubeVirt，并授予应用对 `kubevirt.io/virtualmachines` 的 `create`、`get`、
`deletecollection` 权限。
...
管理后台提供系统配置页面，可查看和更新部分运行配置，也可以触发服务重启。生产环境仍建议将关键部署参数固化在 Helm
values、Secret 或配置管理系统中，避免 Pod 重建后配置漂移。
