...
KubeVirt VM 靶机没有独立开关。题目 compose 中的 service 设置 `x-kubevirt: true` 后，平台会在 VPC 模式下创建 KubeVirt `VirtualMachine`。因此自行部署时还需要安装 KubeVirt，并授予应用对 `kubevirt.io/virtualmachines` 的 `create`、`get`、`deletecollection` 权限。
