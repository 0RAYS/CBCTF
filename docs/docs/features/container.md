...
VM 模式可以在 compose 中保留 `ports` 作为题目作者的服务端口记录，但平台不会读取这些端口创建 Service、NodePort、FRP 转发或访问端点。需要选手访问 VM 服务时，应通过题目网络拓扑、VM 内部代理或其他题目自行设计的方式提供入口；平台只负责创建 VM 和接入 VPC 网络。
