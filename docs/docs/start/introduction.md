...
CBCTF 是一个面向 CTF 比赛的管理与参赛平台。平台提供管理端和选手端，后端使用 Go、Gin、GORM，前端使用 React、Vite，运行时依赖
PostgreSQL 和 Redis；动态附件和动态靶机功能依赖 Kubernetes。
...
如果只使用静态题，平台仍需要 PostgreSQL 和 Redis，但不一定需要 Kubernetes 靶机能力。当前 Helm Chart 默认会在 Kubernetes
中部署应用、PostgreSQL、Redis 和所需 RBAC。
