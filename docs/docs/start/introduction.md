...
CBCTF 后端采用 Go 1.26 + Gin 框架, 前端使用 React 19 + Vite 7, 生产构建产物通过 `go:embed` 嵌入 Go 二进制。平台运行时依赖 PostgreSQL 存储数据、Redis 缓存与任务队列, 并通过 client-go 与 Kubernetes 集群交互以管理容器靶机; 同时集成了 KubeVirt 客户端以支持未来的虚拟机编排。
...
| KubeVirt | 推荐 v1.5+ | 虚拟机编排（实验性, 可选） |

> Kubernetes、KubeOVN 和 Multus 仅在使用动态附件题或容器题时需要。若仅使用问答题和静态题, 只需 PostgreSQL 和 Redis。
