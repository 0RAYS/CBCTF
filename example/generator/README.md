...
4. **生成附件：**
   - 容器内执行命令：
     ...
     ```
   - 生成附件位置为：`/root/attachments/{id}.zip`

5. **将附件复制回主机：**
   ```bash
   cp /root/attachments/{id}.zip /path/in/host/{id}.zip
   ```
...
# 拷贝回主机
cp /root/attachments/1.zip /host/path/1.zip
```
...
4. **附件生成输出要求：**
   - 所有内容必须压缩为 `/root/attachments/{id}.zip`
