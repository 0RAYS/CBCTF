...
func RateLimit(path, ip string, window time.Duration) (int64, model.RetVal) {
	...
	if err != nil {
		...
		return 0, model.RetVal{Msg: i18n.Redis.SetError, Attr: map[string]any{"Key": rateLimitKey, "Error": err.Error()}}
	}
	if count == 1 {
		if err = RDB.Expire(ctx, key, window).Err(); err != nil {
			log.Logger.Warningf("Failed to expire rate limit key: %v", err)
			return 0, model.RetVal{Msg: i18n.Redis.DeleteError, Attr: map[string]any{"Key": rateLimitKey, "Error": err.Error()}}
		}
	}
