...
func SetUserRBAC(userID uint, permissions []string) model.RetVal {
	...
	if err := RDB.Set(ctx, fmt.Sprintf(userRBACKeyTmpl, userID), data, userRBACTTL).Err(); err != nil {
		...
		return model.RetVal{Msg: i18n.Redis.SetError, Attr: map[string]any{"Key": userRBACKeyTmpl, "Error": err.Error()}}
	}
...
func CheckUserRBAC(userID uint, permission string) (bool, model.RetVal) {
	...
	if err != nil {
		if errors.Is(err, redis.Nil) {
			return false, model.RetVal{Msg: i18n.Redis.NotFound, Attr: map[string]any{"Key": userRBACKeyTmpl}}
		}
		log.Logger.Warningf("Failed to get user RBAC permissions: %s", err)
		return false, model.RetVal{Msg: i18n.Redis.GetError, Attr: map[string]any{"Key": userRBACKeyTmpl, "Error": err.Error()}}
	}
...
func DeleteUserRBAC(userID uint) model.RetVal {
	ctx, cancel := context.WithTimeout(context.Background(), time.Minute)
	defer cancel()
	if err := RDB.Del(ctx, fmt.Sprintf(userRBACKeyTmpl, userID)).Err(); err != nil {
		log.Logger.Warningf("Failed to delete user RBAC permissions: %s", err)
		return model.RetVal{Msg: i18n.Redis.DeleteError, Attr: map[string]any{"Key": userRBACKeyTmpl, "Error": err.Error()}}
	}
	return model.SuccessRetVal()
}
...
func DeleteRBAC() model.RetVal {
	...
	defer cancel()
	var cursor uint64
	pattern := strings.TrimSuffix(userRBACKeyTmpl, "%d") + "*"
	for {
