	...
	for magic, userIDs := range deviceUserMap {
		if len(userIDs) > 1 {
			...
			repo.Create(db.CreateCheatOptions{
				...
				Magic:      magic,
				Reason:     fmt.Sprintf(model.SameDeviceMagicTmpl, fmt.Sprintf("User %s", strings.Join(str, ","))),
	...
	for _, submission := range submissions {
		...
		if teamIDL, ok := flagTeamIDMap[submission.Value]; ok && !slices.Contains(teamIDL, submission.TeamID) {
			...
			cheatRepo.Create(db.CreateCheatOptions{
				...
				Reason:     fmt.Sprintf(model.SubmitOtherTeamFlagTmpl, submission.TeamID, strings.Trim(tmp.String(), ", "), contest.ID),
	...
	for ip, users := range ipUserMap {
		if len(users) > 1 {
			...
			cheatRepo.Create(db.CreateCheatOptions{
				...
				IP:         ip,
				Comment:    ip,
				Reason:     fmt.Sprintf(model.ReqWebSameIPTmpl, fmt.Sprintf("User %s", strings.Join(str, ","))),
	...
	for ip, v := range ipTeamMap {
		if len(v) > 1 {
			...
			cheatRepo.Create(db.CreateCheatOptions{
				...
				IP:         ip,
				Comment:    ip,
				Reason:     fmt.Sprintf(model.ReqVictimSameIPTmpl, fmt.Sprintf("Team %s", strings.Join(str, ","))),
