...
func GetTeams(ctx *gin.Context) {
	...
	teams, count, ok, msg := db.InitTeamRepo(DB).ListWithConditions(form.Limit, form.Offset, map[string]any{"contest_id": contest.ID})
	if !ok {
		ctx.JSON(http.StatusOK, gin.H{"msg": msg, "data": nil})
		return
	}
...
//		Solved []model.Flag
//	}
//	flags, _, ok, msg := db.InitFlagRepo(DB).GetByKeyID("contest_id", contest.ID, -1, -1, "Usage", "Usage.Challenge")
