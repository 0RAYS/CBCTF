...
func IncreaseVictimDuration(ctx *gin.Context) {
	...
	victims, ok, msg := repo.GetBy2ID(team.ID, usage.ID, false)
	if !ok {
		ctx.JSON(http.StatusOK, gin.H{"msg": msg, "data": nil})
		return
	}
...
func StopVictim(ctx *gin.Context) {
	team := middleware.GetTeam(ctx)
	usage := middleware.GetUsage(ctx)
	_, msg := service.StopVictim(db.DB.WithContext(ctx), team, usage)
	ctx.JSON(http.StatusOK, gin.H{"msg": msg, "data": nil})
}
...
func GetVictims(ctx *gin.Context) {
	...
	victims, count, ok, msg := repo.GetByTeam(team.ID, form.Limit, form.Offset, true)
	if !ok {
		ctx.JSON(http.StatusOK, gin.H{"msg": msg, "data": nil})
		return
	}
