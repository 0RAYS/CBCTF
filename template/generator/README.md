...
4. 附加至容器, 执行命令 `./run.sh id base64(base64(flag1),base64(flag2),...)` 生成附件, 此处为相对路径, 默认工作路径为 `/root`, 生成的附件在容器中的绝对路径为 `/root/attachments/{id}.zip`
...
# run.sh 将会被传入两个参数, 第一个参数是 id, 第二个参数是 base64(base64(flag1),base64(flag2),...); 参数都当作字符串处理即可
COPY ./files/* /root/
...
3. `run.sh` 务必接收并只能接收两个参数
   ...
   - `base64(base64(flag1),base64(flag2),...)`: 生成附件的 flag, 以 base64 编码传入, 请确保生成的附件可解出该 flag
...
7. 请确保附件可以接收**一定长度的flag**, 以免flag过长导致附件无法解出flag, flag由算法在原模板的基础上进行变异, 以确保flag的随机性
